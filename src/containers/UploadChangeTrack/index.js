...
class UploadChangeTrack extends Component {
  ...
  clearForm = () => {
    this.setState({valueCategory: 0, imgFile: '', trackFile: '', nameTrack: ''})
  }
  ...
  render() {
    ...
    const buttonActions = [ < RaisedButton label = "Clear" style = {{margin: 12}}
      onTouchTap = {
        this.clearForm
      } />, < RaisedButton backgroundColor = "#9E9E9E" label = "Delete" style = {{margin: 12}}
      disabled = {
        this.props.tracks.eventUploadChangeDialog !== 'change'
      }
      onTouchTap = {
        this.deleteTrack
      ...
      onTouchTap = {
        ...
        this.props.tracks.eventUploadChangeDialog == 'upload'
          ? "Add track"
          : 'Save'
      }
      primary = {
      ...
      disabled = {
        this.state.valueCategory == 0 || !this.state.trackFile || !this.state.nameTrack || this.state.disabledButton
      } />
    ];

    return (
      <Dialog title="Upload a track" actions={buttonActions} modal={false} open={this.props.tracks.isOpenedUploadChangeDialogTrack} onRequestClose={this.handleCloseDialog} contentStyle={{
        width: '550px'
      }}>
        <div className='addingTrack'>
            ...
            {this.state.imgFile
              ? (
                <div className="previewImg">
                  <img src={this.state.imgFile.preview
                    ? this.state.imgFile.preview
                    : this.state.imgFile} alt="Preview image"/>
                </div>
              )
              : ''}
          </div>
          ...
          <div className="fieldsTrack">
            <TextField hintText="Name of track" floatingLabelText="Name of track" errorText={this.state.nameTrack == ''
              ? "This field is required"
              : ''} onChange={this.handleNameTrackChange} value={this.state.nameTrack}/>
            <div className="categories">
              <SelectField floatingLabelText="Category" value={this.state.valueCategory} onChange={this.handleCategoryChange} errorText={this.state.valueCategory == 0 && 'Should be chosen'} style={{
                marginTop: '-20px'
              }}>
                {this.props.tracks.arrayCategories.map((category, index) => <MenuItem style={{
                  textTransform: 'capitalize'
                }} key={Date.now() + index} value={category._id} primaryText={category.name}/>)
                }
              </SelectField>
            </div>
          ...
          {this.props.tracks.uploadingTrack
            ? <LinearProgress style={{
                top: '15px'
              }} mode="indeterminate"/>
            : ''}
        </div>
        <Snackbar open={this.props.tracks.uploadedTrackMessage
          ? true
          : false} message={this.props.tracks.uploadedTrackMessage} autoHideDuration={3000} onRequestClose={this.handleSnackbarClose}/>
      </Dialog>
    )
  }
