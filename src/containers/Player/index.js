...
class Player extends Component{
  ...
  componentDidMount(){
    ...
    this.setState({interval: setInterval(this.getCurrentTrackTime, 1000)});
  ...
  getTrackTime = () => {
    ...
    this.setState({trackTimeString: `${min < 10 ? '0'+min : min}:${sec < 10 ? '0'+sec : sec}`, trackTimeSec: Math.ceil(this.state.currentTrack.duration)})
  }
  ...
  getCurrentTrackTime = () => {
    ...
    this.setState({currentTrackTime: `${min < 10 ? '0'+min : min}:${sec < 10 ? '0'+sec : sec}`});
  }
        ...
        <div className="Player">
          <div className="container">
            ...
            <div className={controlsClasses}>
              <MuiThemeProvider>
                <PreviousIcon onClick={this.playerClickPrevious} color='#CCC' viewBox="0 0 25 25" style={{marginLeft: '20px', cursor: 'pointer'}}/>
              </MuiThemeProvider>
              <MuiThemeProvider>
                {this.props.player.isPlayingNow ? (
                  <PauseIcon onClick={this.playerClickPause} color='#CCC' viewBox="0 0 25 25" style={{marginLeft: '15px', cursor: 'pointer'}}/>
                ) : (
                  <PlayIcon onClick={this.playerClickPlay} color='#CCC' viewBox="0 0 25 25" style={{marginLeft: '15px', cursor: 'pointer'}}/>
                )}
              </MuiThemeProvider>
              <MuiThemeProvider>
                <NextIcon onClick={this.playerClickNext} color='#CCC' viewBox="0 0 25 25" style={{marginLeft: '15px', marginRight: '15px', cursor: 'pointer'}}/>
              </MuiThemeProvider>
            </div>
            <div className={progressClasses}>
              <MuiThemeProvider>
                 <Slider onChange={this.changeTimeInPlayingTrack} sliderStyle={{marginTop: '8px', marginBottom: '0'}} min={0} max={this.state.trackTimeSec} defaultValue={0} value={this.state.currentTrack.currentTime}/>
              </MuiThemeProvider>
            </div>
            <div className={timeClasses}>
              {this.state.currentTrackTime} / {this.state.trackTimeString}
            </div>
            ...
            <div className={progressVoiceClasses}>
              <MuiThemeProvider>
                 <Slider onChange={this.changeVolume} sliderStyle={{marginTop: '8px', marginBottom: '0'}} min={0.0} max={1.0} defaultValue={0.5} />
              </MuiThemeProvider>
            </div>
          </div>
        </div>
